...
$addslashed_audit=_nodesforum_my_custom_addslashes($_GET['_nodesforum_audit']);
//get info about post or folder to audit
$result = mysql_query("SELECT creator_uniqueID, creator_ip, creation_time, folder_or_post, containing_folder_or_post, ancestry, skeleton FROM ".$_nodesforum_db_table_name_modifier."_nodesforum_folders_and_posts WHERE fapID = '$addslashed_audit'");
...
// $issure=0;
// if(!isset($_GET['_nodesforum_audit_user']) && !isset($_GET['_nodesforum_delete_ip']))
// {$issure=1;}
// else
// {
...
//if has the right to audit..
if($_nodesforum_righttoaudit==1){
    $audit_authority=$_nodesforum_modship_authoritative_folder;
    ...
    // if(isset($_GET['_nodesforum_audit_user']))
    // {$mods_log_retrieval_number='spcus_'.$mods_log_retrieval_number;}
    // else if(isset($_GET['_nodesforum_audit_ip']))
    ...
    // {
        ...
        //audit this folder or post
        mysql_query("UPDATE ".$_nodesforum_db_table_name_modifier."_nodesforum_folders_and_posts SET audited = 1 WHERE fapID = '$addslashed_audit'");
	...
	if(isset($_GET['format']) && $_GET['format']=='json'){
        ...
        //$location.='#_nodesforum_anchor_'.$_GET['_nodesforum_audit'];
		header($location);
	}
}
